...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/05 15:53:02 by dda-fons          #+#    #+#             */
/*   Updated: 2025/12/05 15:53:03 by dda-fons         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (++m.my <= py + MM_RAD)
	{
		...
		while (++m.mx <= px + MM_RAD)
		{
			...
			if (game->grid.map[m.my][m.mx] == '1')
				minimap_draw_wall(game, px, py, &m);
			else if (game->grid.map[m.my][m.mx] == 'C')
				minimap_draw_collectable(game, px, py, &m);
			else if (game->grid.map[m.my][m.mx] == 'D')
				minimap_draw_door(game, px, py, &m);
		}
	}
}
